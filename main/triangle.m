...
for k=1:num_load
    ...
    P_xyt_exp=Pm*((x+a/2-Cx*t+lr+ld)/ld);     %exponential part
    for i=1:n
        fprintf('k =%d i= %d \n',k,i);
        for j=1:n
            if t<=tr
            ...
            elseif t>tr&& t<tr+td
                fun = matlabFunction(P_xyt_exp*W_base{i,j});            %exponential part
                aaa2=integral2(fun,-a/2,Cx*t-lr-a/2,-b/2,b/2,'AbsTol',1e-3);
                fun = matlabFunction(P_xyt_line*W_base{i,j});           %linear part
                aaa1=integral2(fun,Cx*t-lr-a/2,Cx*t-a/2,-b/2,b/2,'AbsTol',1e-3);
                aaa=aaa1+aaa2;
            elseif t>tr+td&& t<=a/Cx
                fun = matlabFunction(P_xyt_exp*W_base{i,j});            %exponential part
                aaa2=integral2(fun,Cx*t-lr-a/2,Cx*t-a/2,-b/2,b/2,'AbsTol',1e-3);
                fun = matlabFunction(P_xyt_line*W_base{i,j});           %linear part
                aaa1=integral2(fun,Cx*t-lr-ld-a/2,Cx*t-lr-a/2,-b/2,b/2,'AbsTol',1e-3);
                aaa=aaa1+aaa2;
            elseif t>a/Cx&& t<=a/Cx+tr
                fun = matlabFunction(P_xyt_exp*W_base{i,j});            %exponential part
                aaa2=integral2(fun,Cx*t-lr-a/2,a/2,-b/2,b/2,'AbsTol',1e-3);
                fun = matlabFunction(P_xyt_line*W_base{i,j});           %linear part
                aaa1=integral2(fun,Cx*t-lr-ld-a/2,Cx*t-lr-a/2,-b/2,b/2,'AbsTol',1e-3);
                aaa=aaa1+aaa2;                
            elseif t>a/Cx+tr&& t<=a/Cx+tr+td
                fun = matlabFunction(P_xyt_exp*W_base{i,j});            %exponential part
                aaa=integral2(fun,Cx*t-lr-ld-a/2,a/2,-b/2,b/2,'AbsTol',1e-3);
            end
            PA_t_num(i,j,k)=aaa;
